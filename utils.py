...
# Written by Ze Liu
# Modified by Zhenda Xie And Akihito Maruya Modified
# --------------------------------------------------------
...
def load_checkpoint(config, model, optimizer, lr_scheduler, logger):
    ...
    if config.MODEL.RESUME.startswith('https'):
        ...
        # ---- FIX for PyTorch 2.6+ ----
        # torch.load() now defaults to weights_only=True, which fails for checkpoints
        # containing non-tensor objects like yacs.config.CfgNode stored under 'config'.
        # Setting weights_only=False restores the old behavior without changing logic.
